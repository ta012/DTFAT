...
def load_pretrained_apr22(
    ...
    if classifiers is not None:
        ...
        if num_classes != pretrained_cfg['num_classes']:
            for classifier_name in classifiers:
                print(f"\n pop {classifier_name + '.weight'}\t{classifier_name + '.bias'}")
    ...
    print("averaging input channel 3 kernel channel 1 ")
    ...
    for k,v in state_dict.items():
        if 'stem.' in k:
            ...
            if not old_k_f in model.state_dict().keys(): 
                ...
                exit('error')
            ## reshaping kernal
            print(f"Stem param {k} weight {v.shape}")
            v_t=copy.deepcopy(v)
            v_f=copy.deepcopy(v)
            v_t_shape = model.state_dict()[old_k_t].shape[-2:]

            if v_t_shape==v.shape[-2:]:
                print("Updating without reshaping",old_k_t, v_t.shape)
                print("Updating without reshaping",old_k_f, v_f.shape)
            ...
            print("Updating after reshaping",old_k_t, v_t.shape)
            print("Updating after reshaping",old_k_f, v_f.shape)
            state_dict_new1[old_k_t] = v_t
            state_dict_new1[old_k_f] = v_f
    state_dict = state_dict_new1
    #######################
    ...
    for k,v in state_dict.items():
        ...
        if '.conv2_kxk.' in k:
            ...
            print(f"\nUpdating key {k, old_k_t,old_k_f}")
    ...
    print("relative_pos_keys, ", relative_pos_keys)
    ...
    print("\n All the keys in params in model.state_dict except relative pos keys head.fc are missing so good to load with strict =", strict)
